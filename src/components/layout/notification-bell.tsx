...
export function NotificationBell() {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <>
          {/* Click-outside backdrop. The dropdown itself sits above this (z-50)
              so pointer events inside the dropdown bubble to their targets
          ...
          <div className="fixed left-2 right-2 top-14 sm:left-auto sm:right-0 sm:top-full sm:mt-2 sm:w-96 bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-white/10 bg-white dark:bg-gray-900">
              ...
              {unread > 0 && (
                <button
                  onClick={markAllRead}
                  className="text-xs text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300 flex items-center gap-1"
                >
            ...
            <div className="max-h-[70vh] sm:max-h-96 overflow-y-auto divide-y divide-gray-100 dark:divide-white/5 bg-white dark:bg-gray-900">
              {notifications.length === 0 ? (
                ...
                notifications.map((n) => {
                  const inner = (
                    <div className={`px-4 py-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors ${!n.is_read ? 'bg-indigo-50 dark:bg-indigo-950/30' : ''}`}>
                      <div className="flex items-start gap-3">
                        <div className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${!n.is_read ? 'bg-indigo-500 dark:bg-indigo-400' : 'bg-transparent'}`} />
            ...
            <div className="px-4 py-2 border-t border-gray-100 dark:border-white/10 text-center bg-white dark:bg-gray-900">
              <Link
