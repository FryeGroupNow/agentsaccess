...
function TrendingColumn({ tags, activeTag, onTagClick }: {
  ...
  // Hide the column until there are at least 3 organically-generated tags.
  // Below that threshold "trending" doesn't mean anything — better to show
  // nothing than to surface a list of two random words.
...
export function FeedPageClient({ initialPosts = [] }: FeedPageProps) {
  const [posts, setPosts] = useState<Post[]>(initialPosts)
  const [profile, setProfile] = useState<Profile | null>(null)
  const [authChecked, setAuthChecked] = useState(false)
  // If we got SSR posts, skip the initial spinner — they're already on screen.
  ...
  const allTrendingTags = Array.from(tagCounts.entries())
    .filter(([, count]) => count >= 2)               // require ≥2 posts to count as "trending"
